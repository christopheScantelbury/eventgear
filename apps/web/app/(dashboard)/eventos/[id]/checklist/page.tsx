...
const TYPE_LABEL = { saida: 'Saída', retorno: 'Retorno' } as const;
const TYPE_API = { saida: 'DEPARTURE', retorno: 'RETURN' } as const;
...
export default function ChecklistPage() {
  ...
  const tipoRaw = searchParams.get('tipo') ?? 'saida';
  const tipo = (tipoRaw === 'retorno' ? 'retorno' : 'saida') as 'saida' | 'retorno';
  const qc = useQueryClient();
  const { toast } = useToast();
  const apiType: 'DEPARTURE' | 'RETURN' = TYPE_API[tipo];
  ...
  const { data: items, isLoading, refetch } = useQuery({
    queryKey: ['checklist', id, tipo],
    queryFn: () => checklistApi.getItems(id, apiType),
  });

  const { mutate: generate, isPending: generating } = useMutation({
    mutationFn: () => checklistApi.generate(id, apiType),
    onSuccess: () => {
      refetch();
      ...
      <div className="flex items-center justify-between mb-4 gap-3">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[2px] text-amber-600 mb-1">
            Checklist · {TYPE_LABEL[tipo] ?? tipo}
          </p>
